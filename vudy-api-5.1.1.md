...
1. [Application Management API](#application-management-api)
2. [Authentication API](#authentication-api)
3. [Channels API (v1)](#channels-api-v1)
4. [Configuration API](#configuration-api)
5. [KYC Verification API](#kyc-verification-api)
6. [Notifications API](#notifications-api)
7. [Organization Management API](#organization-management-api)
8. [OTC Operations API](#otc-operations-api)
9. [Profile Management API](#profile-management-api)
10. [Team Management API](#team-management-api)
11. [Transaction History API](#transaction-history-api)
12. [Vudy Request API](#vudy-request-api)
13. [Wallet Management API](#wallet-management-api)
14. [README](#readme)
15. [Authentication Flow](#authentication-flow)
16. [Onboarding Flow](#onboarding-flow)
17. [External Wallets Flow](#external-wallets-flow)
18. [Vudy Request Create Flow](#vudy-request-create-flow)
19. [Vudy Send Tokens (Executive Summary)](#vudy-send-tokens-executive-summary)
20. [Vudy Send Tokens Flow](#vudy-send-tokens-flow)
21. [Vudy OTC Request Flow](#vudy-otc-request-flow)
22. [API Key Webhooks](#api-key-webhooks)
23. [Auth Patterns](#auth-patterns)
24. [Logged-in Profile View: Data Map](#logged-in-profile-view-data-map)
25. [Response Format](#response-format)
26. [Vudy Requests User Flow](#vudy-requests-user-flow)
...
> Source: `app/v1/channels/README.md`
...
-   [Base URL](#base-url)
-   [Endpoints](#endpoints)
    -   [Get Channel Fees](#get-channel-fees)
-   [Error Response Format](#error-response-format)
-   [Support](#support)

## Overview

The Channels API (v1) provides endpoints for retrieving channel-specific configurations and fee structures. This API enables applications to query fee information for specific channels and methods.

## Authentication

Endpoints support both API key only and API key + session authentication.
...
**Note:** When using API key only, the API key must have a `teamId` associated with it.
...
**Note:** When using API key + session, the teamId is retrieved from the session's current team or the first profile's team.
...
Retrieves the effective fee configuration for a specific channel and team. The fee structure combines application-level and team-level fee configurations with the following merging rules:
- If a property exists in both app and team fees, team value is used (team overrides)
- If a property doesn't exist in team fees, app value is used
- If a property exists in team fees but not in app fees, an error is returned
- Recipients are always excluded from the response (admin-only information)

**Endpoint:** `GET /v1/channels/fees`
...
```json
{
    "x-api-key": "string (required, must have teamId)"
}
```
...
| ----------- | ------ | -------- | --------------------------------------------------------------------------- |
| channelSlug | string | Yes      | Channel slug (e.g., "vudy") |

#### Example Request

**API Key Only:**
```bash
curl -X GET "https://api.vudy.app/v1/channels/fees?channelSlug=vudy" \
...
The response contains the merged fee configuration with the following structure:

-   `[methodName]` (object, optional) - Method-specific fee configuration:
    -   `type` (string) - Fee type: `"fixed"` (fixed amount) or `"percentage"` (basis points)
    -   `style` (string) - Fee style: `"surcharge"` (added on top) or `"commission"` (deducted from amount)
    -   `value` (number) - Fee value:
        -   For `"fixed"`: Amount in token's smallest unit
        -   For `"percentage"`: Basis points (1 basis point = 0.01%, so 50 = 0.5%)
-   `minSurchargeBps` (number) - Minimum surcharge in basis points
-   `minCommissionBps` (number) - Minimum commission in basis points

**Note:** The `recipients` field is always excluded from the response as it is admin-only information.

#### Error Codes

-   `1` - Missing channelSlug parameter (400)
-   `2` - Channel not found (400)
-   `3` - No teamId available from API key or session (400)
-   `4` - Failed to get fee configuration (500)
-   `5` - Channel fee configuration not found for app (500)
-   `6` - Team fee property not found in app fees (500)
...
```json
{
    ...
    "error": {
        "code": "1",
        "message": "Missing channelSlug parameter",
        "details": {}
    }
}
```
...
```json
{
    ...
    "error": {
        ...
        "details": {
            "channelSlug": "invalid-channel"
        }
    }
}
```

**No Team ID Available (400)**
...
```json
{
    ...
    "error": {
        "code": "4",
        "message": "No teamId available from API key or session",
        "details": {}
    }
}
```

**Failed to Get Fee (500)**
...
```json
{
    ...
    "error": {
        ...
        "details": {
            "channelId": "vudy",
            "teamId": "team-uuid",
            "error": "Channel fee configuration not found for app"
        }
    }
}
```

**Channel Fee Not Found (500)**
...
```json
{
    ...
    "error": {
        "code": "5",
        "message": "Channel fee configuration not found for app",
        "details": {
            "channelId": "vudy",
            "appId": "app-uuid",
            "teamId": "team-uuid"
...
**Team Fee Property Not Found in App (500)**
...
```json
{
    ...
    "error": {
        "code": "6",
        "message": "Team fee property 'customMethod' not found in app fees",
        "details": {
            "channelId": "vudy",
            "appId": "app-uuid",
            "teamId": "team-uuid",
